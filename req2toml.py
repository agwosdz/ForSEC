...
def main(argv: list[str] | None = None) -> int:
    ...
    # ---- Separate direct URL refs → post-install ----
    normal_deps: list[str] = []
    direct_ref_deps: list[str] = []
    for dep in result.deps:
        if re.search(r"\s*@\s+https?://", dep):
    ...
    if pt_index:
        pt_pkgs = _find_pytorch_packages(normal_deps)
        for pkg in pt_pkgs:
    ...
    if args.dry_run:
        ...
        # Generate post_install.cmd for direct-ref deps + remaining commands
        post_install_lines: list[str] = []

        # Direct URL deps → uv pip install
        for dep in direct_ref_deps:
            post_install_lines.append(f"%UV% pip install {dep}")

        # Remaining non-pip commands
        for cmd_line in remaining_commands:
            ...
            post_install_lines.append(cmd_line)

        if post_install_lines:
            post_install = args.output.parent / "post_install.cmd"
            # Header: resolve uv path (same logic as setup_uv.cmd)
            lines_out = [
                "@echo off",
                ...
                "",
                "REM Resolve uv path — prefer PATH, fallback to full path",
                'set "UV=uv"',
                "where uv >nul 2>&1",
                "if %ERRORLEVEL% neq 0 (",
            ...
            for cmd_line in post_install_lines:
                ...
                lines_out.append('')
            lines_out.append("endlocal")
            post_install.write_text("\n".join(lines_out), encoding="utf-8")
            print(f"Wrote {post_install}")

        if direct_ref_deps:
            print(
                f"\n⚠  {len(direct_ref_deps)} direct URL dep(s) moved to post_install.cmd "
                f"(must run after uv sync):",
                file=sys.stderr,
            )
            for d in direct_ref_deps:
